 ...
 */
$(function() {
	$('#experienceDataGrid'+$('#companyId').val()).datagrid({
		url :_path + '/shopCheck/findExperience?companyId='+$('#companyId').val(),
		columns : [ [ {
			field : 'modifyTime',
			sortable : true,
			halign: 'center',
	...
	*/
	...
	//保证金
	$('#earnestDataGrid'+$('#companyId').val()).datagrid({
		url :_path + '/earnestManager/findList?companyId='+$('#companyId').val(),
		columns : [ [ {
			field : 'addtime',
			sortable : true,
			halign: 'center',
...
function saveExperience(){
	if ($('#experienceForm'+$('#companyId').val()).form('validate')) {
		var param_data = biz.serializeObject($("#experienceForm"+$('#companyId').val()));
		...
		$.post(_path + '/shopCheck/saveExperience', param_data, function(_data) {
			if (_data.head.retCode == '000000') {
				...
				var ddv = $("#shopCheckDataGrid").datagrid('getRowDetail', $('#datagrid_index').val()).find('div.shopCheckDetail');
				ddv.panel('refresh');
			}else {
				easyuiMsg('错误', "保存失败！");
			}
...
function saveEarnest(){
	if ($('#earnestForm'+$('#companyId').val()).form('validate')) {
		var param_data = biz.serializeObject($("#earnestForm"+$('#companyId').val()));
		param_data.companyId=$('#companyId').val();
...
function loadPicture(){
	$.each($('.shop-confirm img'),function(i,that){
