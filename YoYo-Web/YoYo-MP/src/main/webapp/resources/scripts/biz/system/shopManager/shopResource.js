...
$(function() {
	$('#table-resource-manager').datagrid({
		url : _path + '/shopResource/shopResourceList',
		columns : [ [ {
			field:'ck',
			checkbox:"true"
		},{
...
function saveResource() {
	if ($('#form-resource-manager-add').form('validate')) {
		...
		if($("#id").val()=='' || $("#id").val()==null){ //添加
			$.post(_path + '/shopResource/insertShopResource', param_data, function(_data) {
			...
			$.post(_path + '/shopResource/updateShopResource', param_data, function(_data) {
				if (_data.head.retCode == '000000') {
					$('#table-resource-manager').datagrid('reload', {});
					$('#window-add-resource-manager').window('close');
				} else {
...
function deleteResource(){
	...
	$.messager.confirm('确认','您确认想要删除数据项吗？',function(r){    
		    if (r){  
		    	$.ajax({
		    	    url:_path + '/shopResource/deleteShopResource',
		    	    type:"post",
		    	    data:params,
		    	    dataType:"json",
