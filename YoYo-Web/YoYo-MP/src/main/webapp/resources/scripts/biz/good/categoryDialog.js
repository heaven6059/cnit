				...
				$.ajax({ url : para.url , type : "post" , data : arg_data , datatype : "json" , success : function(data) {
					if (data.length > 0) {
						...
						if (data.length >= 10) { // 大于10条就显示查询框
							initHtml += '<li class="filter_item"><input autofocus="" id="' + data[ 0 ].catId + '-filter' + '" class="ipt_search ui-widget" placeholder="输入拼音首字母、关键字"';
							initHtml += 'style="color: rgb(0, 0, 0);"></li>';
						} 
						$.each(data, function(i, n) {
							if (n.childCount > 0) { // 存在下级分类
								initHtml += '<li class="categoryIsParent"  cat_id="' + n.catId + '" identifier="'+n.identifier+'">';
							} else {
								initHtml += '<li class="categoryIsLeaf" cat_id="' + n.catId + '"  identifier="'+n.identifier+'">';
							}
			...
			var getBrandInfo = function(arg_data) {
				$.ajax({ url : _path + '/brandTypeShip/findBrandCateShip' , type : "post" , data : arg_data , datatype : "json" , success : function(result) {
					...
					if (data.length > 0) {
						var initHtml = '<ul class="cat_select">';
						if (data.length >= 10) { // 大于10条就显示查询框
							initHtml += '<li class="filter_item"><input autofocus="" id="' + data[ 0 ].brandId + '-filter' + '" class="ipt_search ui-widget" placeholder="输入拼音首字母、关键字"';
							initHtml += 'style="color: rgb(0, 0, 0);"></li>';
						}
						$.each(data, function(i, n) {
							initHtml += '<li class="brandIsLeaf" brand_id="' + n.brandId + '">';
							initHtml += n.brandName + '<span class="icon"></span></li>';
						});
						initHtml += '</ul>';
						$('.goods_category_body').append(initHtml);
						// 绑定品牌选择事件
