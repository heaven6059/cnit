...
function initList(){
	$('#table-store-violation').datagrid({
		url : _path + '/storeViolation/findStoreViolationList',
		columns : [ [ {
			field:'ck',
			checkbox:"true",
			width:'2%'
...
function openSeachWindow() {
	...
	$('#advance_search_store_violation').dialog('open').window('resize',{ width : '380px' , height : '500px' , top : 0 , left : (width - 380) });
}
//清空高级选项条件
