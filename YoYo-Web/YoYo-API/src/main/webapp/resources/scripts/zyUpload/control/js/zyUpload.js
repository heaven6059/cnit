	...
	$.fn.zyUpload = function(options,param){
		...
		//            html += '					<div class="webuploader_pick">继续选择</div>';
				...
				//	html += '		<div id="uploadInf" class="upload_inf"></div>';
		            ...
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">选中0张文件，共0B。第一张图片为默认图片</div>';
		            html += '				<div class="btns">';
		         //   html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		            html += '					<input id="fileImage" type="file" size="30" name="imageFile" '+multiple+'>';
		        ...
		        //    html += '					<div class="upload_btn">开始上传</div>';
				    ...
				    html += '							<img class="upload_image" src="'+para.path+'/resources/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						...
						var funAppendPreviewHtml = function(html){
							...
							if(para.dragDrop){
								...
								if($('.upload_append_list').length >=3){
									$(".add_upload").hide();
								}
							}
							// 绑定删除按钮
					...
					onSuccess: function(file,response) {
						...
						$("#uploadSuccess_" + file.index).attr("picAddress",response);	
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
