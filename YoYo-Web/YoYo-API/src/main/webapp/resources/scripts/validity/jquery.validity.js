...
(function($, undefined) {
	...
	$.validity = {
		...
		patterns : {
			...
			// Used to use Date.parse(), which was the cause of Issue 9, where
			// the
			// function would accept 09/80/2009 as parseable. The fix is to use
			// a
			// RegExp that will only accept American Middle-Endian form. See the
			// Internationalization section in the documentation for how to
			// cause
			...
			// negative. It should also support scientific-notation, written as
			// a
			// lower or capital 'E' followed by the radix. Number assumes base
			// 10.
		...
		// Character classes can be used to determine the quantity of a given
		// type
		...
		// starts, this will be set to a report object. When validators fail,
		// they
		// will inform this object. When validation is completed, this object
		// will
	...
	$.fn.extend({
		...
		// The validity function is how validation can be bound to forms. The
		// user
		// may pass in a validation function or, as a shortcut, pass in a string
		// of
		...
		greaterThan : function(min, msg) {
			...
			min.getTime ? orEmpty(function(obj) {
		...
		lessThan : function(max, msg) {
			...
			max.getTime ? orEmpty(function(obj) {
		...
		maxLength : function(max, msg) {
			return validate(this, orEmpty(function(obj) {
		...
		equal : function(arg0, arg1) {
			...
			if ($reduction.length) {
				...
				// If any value is not equal to the first value, then they
				// aren't
			...
			if ($reduction.length) {
				...
				if (!valid) {
					...
					for ( var i = 0, repeatedLength = repeatedVal.length; i < repeatedLength; ++i) {
						...
						// raise the error - aggregate will use last repeated
						// value
	...
	// Using the associative array supplied as the 'obj' argument, replace
	// tokens
	...
	// Infer the field name of the passed DOM element. If a title is specified,
	// its
...
(function($) {
	$.validity.outputs.tooltip = {
		...
		raise : function($obj, msg) {
			...
			var tooltip = $("<div class=\"validity-tooltip\">" + msg + "<div class=\"validity-tooltip-outer\">" + "<div class=\"validity-tooltip-inner\"></div>" + "</div>" + "</div>").click(function() {
				$obj.focus();
				$(this).fadeOut();
...
(function($) {
	...
	$.validity.outputs.label = {
		...
		start : function() {
			...
			$("label." + $.validity.outputs.label.cssClass).remove();
		},
		...
		raise : function($obj, msg) {
			...
			// Otherwize create a new one and stick it after the input:
				...
				var errorLabel = $("<label/>").attr("for", getIdentifier($obj)).addClass($.validity.outputs.label.cssClass).text(msg)
				...
				// (if it exists)
				if ($obj.is(":checkbox") && $("label[for='" + getIdentifier($obj) + "']").length) {
...
(function($) {
	...
	$.validity.outputs.summary = {
		options : {
			// Configurable container selector to facilitate having multiple
			// containers on one page
		...
		start : function() {
			...
			$(this.options.container).stop().hide().find("ul").html('');
			...
			if (buffer.length) {
				...
				for ( var i = 0; i < buffer.length; ++i) {
					$(wrapper).text(buffer[i]).appendTo(this.options.container + " ul");
				}
...
// Install the layer output.
(function($) {
	var
	// Class to apply to layer errors.
	errorClass = "validity-layer-msg";

	$.validity.outputs.layer = {
		start : function() {
			// Remove all the existing errors.
			$("." + errorClass).remove();
		},
