...
$(function() {
	...
	$("#navScrollLeft li").click(function (){
		console.log($(this).index);
		$(this).siblings().children().removeClass("active");
		$(this).children().addClass("active");
		var x=$("#"+$(this).find("a").attr("data-target")).offset().left;
		var y=$("#"+$(this).find("a").attr("data-target")).offset().top;
		window.scrollTo(x,y);				
	});
	...
	$('.js-btn-delete').click(function (){
		var target=$(this).closest("td"),goodsId=$(this).attr("data-goods"),cookieV = utils.readCookie("accessory_compare"),compareArray=[];		
		if(cookieV){
			$.each(JSON.parse(cookieV),function (x,item){
				if(item.id!=goodsId){
					compareArray.push(item);
				}
			});
			utils.writeCookie("accessory_compare", JSON.stringify(compareArray));
		}
...
function init(){
	var cookieV=utils.readCookie("accessory_compare");
	if(cookieV){
		$.each(JSON.parse(cookieV),function (x,item){		
				var opts={
					brand:item.brandId,
