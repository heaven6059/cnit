				...
				if(data.content.sameOrderProduct!=null&&data.content.sameOrderProduct.length>=1){
					...
					for(var i =0; i<sameOrderList.length;i++){
						$('.add_left .add_in').eq(0).find('.shop_con ul').append(''
								+'<li>'
									+'<div class="p_img fl">'
										+'<a href="'+yoyo.portalUrl+'/goodsManager/goodsIndex?goodsId='+sameOrderList[i].goodsId+'" target="_blank"><img src="'+goodsImgSrcPre+sameOrderList[i].picturePath+'" alt="'+sameOrderList[i].productName+'" width="100px" height="100px"></a>'
									+'</div>'
									+'<div class="fr" style="width:192px;height:100px;">'
										+'<h5 style="height:44px;overflow:hidden;"><a href="'+yoyo.portalUrl+'/goodsManager/goodsIndex?goodsId='+sameOrderList[i].goodsId+'" target="_blank">'+sameOrderList[i].productName+'</a></h5>'
										...
										+'<span><a class="p_btn" href="'+yoyo.portalUrl+'/goodsManager/goodsIndex?goodsId='+sameOrderList[i].goodsId+'">加入购物车</a></span>'
									+'</div>'
								+'</li>'
								+'');
//						alert(i);
