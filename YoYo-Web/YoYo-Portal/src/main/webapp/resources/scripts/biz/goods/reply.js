		...
		commonAjax(url, params, function(data) {
			...
			if(data.content!=null&&data.content.rows.length>=1){
				...
				$.each(data.content.rows,function (x,item){
					html.push('<div style="" class="item-reply none">');
					html.push('<div class="reply-list">');
					html.push('<div class="reply-con">');
					html.push('<span class="u-name"><a href="javascript:;" rel="nofollow">'+item.loginName+'</a>('+item.commentDate+')<em>回复 </em><a href="javascript:;" rel="nofollow">'+item.toName+'</a>：</span>');
					html.push('<span class="u-con">'+item.comment+'</span>');
					html.push('</div>');
					html.push('</div>');
					html.push('</div>');
				});
				$("#replyList").html(html.join(""));
