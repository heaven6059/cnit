 ...
 * @returns
 */
function getQQAuthorizeURL(thirdType) {
	var callback=_url +"connectsign/qqlogin"+"?thirdType="+thirdType+"&accountType="+$('#id_accountType').val();
...
function getAuthorizeURL(thirdType) {
	var callback = _url +"connectsign/afterQQLogin";
...
var my_car_utils={
	...
	addCarFailure:function (){
		$(".header_car_save_box").show();
		$(".header_other_box").hide();
		/**
		var area=["280px","20px"];
		if(getOs()=="Safari"){
			area=["280px","25px"];
		}
		layer.tips("车型信息未保存,<a style='color:#ddd;font-size:14px;font-weight:bloder' href='"+yoyo.portalUrl+"/register/login'>登录</a>或<a style='color:#ddd;font-size:14px;font-weight:bloder' href='"+yoyo.portalUrl+"/register/signup'>注册</a>后将会自动保存", '#addMyCar', {
			tips:3,
			time:false,
			closeBtn :1,
			zIndex:98,
			...
			success: function(layero, index){						
		        ...
		        $(layero).css({left:left,"font-size":"12px",top:"190px"});
	...
	readMyCar:function(){
		...
		if(my_car_utils.readCookie(my_car_utils.is_login)){
			$.ajax({url:yoyo.memUrl+"/membercar/findMemberDefaultCar",type:"post",cache:false,dataType:'json',success:function(data){
 ...
 * @param $obj
 */
function validateSmsCode(smsObj,mobileObj, sucCallback, failCallback) {
	var $smsObj = $('#' + smsObj);
	var $mobileObj = $('#' + mobileObj);
	var params = {
	        "smsCode": $smsObj.val(),
	        "mobile": $mobileObj.val()
	    };
	if ($smsObj && $smsObj.val()) {
		commonAjax(_path + "/oauth/validateSmsCode", params, sucCallback, function(data) {
			renderMsg($obj, data.retMsg);
			failCallback(data);
		});
	} else {
...
function renderDialog(msg) {
	layer.open({
	    ...
	    shift : 2, 
	    shadeClose : true,
	    content: '<div style="padding-top: 8px; padding-left: 10px;font-size:14px">'+msg+'</div>'
	});  
}
/**
