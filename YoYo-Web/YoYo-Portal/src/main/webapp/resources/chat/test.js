// XMPP服务器BOSH地址
var BOSH_SERVICE = 'http://127.0.0.1:7070/http-bind/';// XMPP连接
...
function onConnect(status) {    
	...
	if (status == Strophe.Status.CONNFAIL) {        
		...
		connection.addHandler(onMessage, null, 'message', null, null, null);                // 首先要发送一个<presence>给服务器（initial presence）        
		connection.send($pres().tree()); 
	}
}// 接收到<message>

function onMessage(msg) {        // 解析出<message>的from、type属性，以及body子元素    
