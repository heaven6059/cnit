...
//增加对ie8的支持  by caizhijie
var EventUtil = {
	addHandle:function(ele,type,handler){
...
var ZYFILE = {
		...
		funUploadFiles : function(){
			var self = this;  // 在each中this指向没个v  所以先将this保留
			// 遍历所有文件  ，在调用单个文件上传的方法
			//2015.06.29 xiaox 改成只传单个文件
			/*$.each(this.uploadFile, function(k, v){
				self.funUploadFile(v);
			});*/
			if(this.uploadFile.length>0){
				self.funUploadFile(this.uploadFile[this.uploadFile.length-1]);
			}
		},
		// 上传单个个文件
		...
		//	formdata.append("fileList", file);      xiaox 修改 		
			formdata.append("imageFile", file);
			var xhr = new XMLHttpRequest();
			// 绑定上传事件
			// 进度
			EventUtil.addHandle(xhr.upload, 'progress', function(e){
		    	// 回调到外部
...
//		    }, false); 
		    // 完成
			EventUtil.addHandle(xhr, 'load', function(e){
...
//		    }, false);  
		    // 错误
			EventUtil.addHandle(xhr, 'error', function(e){
		    	// 回调到外部
...
//		    }, false);  
			...
			//TODO xhr.setRequestHeader("X_FILENAME", file.name);
			xhr.send(formdata);
		},
		// 返回需要上传的文件
		...
		init : function(){  // 初始化方法，在此给选择、上传按钮绑定事件
			...
			if (this.dragDrop) {
				EventUtil.addHandle(this.dragDrop, 'dragover', function(e) { self.funDragHover(e); });
				EventUtil.addHandle(this.dragDrop, 'dragleave', function(e) { self.funDragHover(e); });
				EventUtil.addHandle(this.dragDrop, 'drop', function(e) { self.funGetFiles(e); });
...
//				this.dragDrop.addEventListener("drop", function(e) { self.funGetFiles(e); }, false);
			...
			if(self.fileInput){
				// 绑定change事件
				EventUtil.addHandle(this.fileInput, 'change', function(e) { 
					console.log("222=");
					console.log(e);
					self.funGetFiles(e); 
				});
...
//				}, false);	
			...
			if(self.uploadInput){
				// 绑定click事件
				EventUtil.addHandle(this.uploadInput, 'click', function(e) { 
