	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.createHtml = function(){
	          ...
	          //  	html += '						<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
	              	html += '						<input id="fileImage" type="file" size="30" name="imageFile" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
		...
		//            html += '					<div class="webuploader_pick">继续选择</div>';
				...
				//	html += '		<div id="uploadInf" class="upload_inf"></div>';
		            ...
		            html += '			<div class="status_bar">';
		            html += '				<div id="status_info" class="info">第一张图片为默认图片</div>';
		            html += '				<div class="btns">';
		         //   html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		            html += '					<input id="fileImage" type="file" size="30" name="imageFile" '+multiple+'>';
		        ...
		        //    html += '					<div class="upload_btn">开始上传</div>';
				    ...
				    html += '							<img class="upload_image" src="'+yoyo.shopUrl+'/resources/images/add_img.png" style="width:expression(this.width > '+imgWidth+' ? '+imgWidth+'px : this.width)" />';
				    html += '						</div>';				    
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
			...
			this.funSetStatusInfo = function(files){
				...
				// 设置内容
				//$("#status_info").html("选中"+num+"张文件，共"+size+"。第一张图片为默认图片");
				$("#status_info").html("第一张图片为默认图片");
			};
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '	<p id="uploadDefault_'+file.index+'" class="file_default">默认图</p>';
					html += '</div>';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						if(selectFiles[0].size>2*1024*1024){  //xiaox  添加
							parent.$.messager.alert('提示', '上传失败！文件大小不能超过2M', 'error');
							return false;
						}
						...
						funDealtPreviewHtml();	
						$("#fileSubmit").click();  //xiaox 添加，上传文件
					},
					onDelete: function(file, files) {
						// 移除效果
						//2015.05.05 xiaox
						var pictureId = $("#uploadSuccess_" + file.index).attr('pictureId');
						var isChooseCount = $(".img_"+pictureId).length;
						if(isChooseCount>0){
							easyuiMsg('提示', "该图片已经关联规格，请取消关联关系，再删除！");
							return false;
						}else{
							para.onDelete(file, files); // 删除选择货品图片
						}
					...
					onSuccess: function(file,response) {
						...
						$("#uploadSuccess_" + file.index).attr('pictureId',pic[0]);  //2015.05.05 xiaox
