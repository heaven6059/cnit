...
function loadDataList(opts){
	...
	$.ajax({
		url:'../fullReduce/getFullDiscountList',
		type:"post",
		dataType:"json",
		async: false,
		success:function(returnData){
			   $.each(returnData.rows,function(index,activity){
         		...
         		html.push('<td><a href="../fullReduce/editDiscount/'+activity.actId+'" >编辑</a>|<a href="javascript:void(0);" onclick ="deleteActivity('+activity.actId+","+activity.ruleId+')">删除</a></td><br>');
	            	html.push("</tr>");	          
			   });
...
function addBtn(){
	window.location.href = _path + '/fullReduce/addfullDicountRule';
}

function deleteActivity(actId,ruleId) {
	$.messager.confirm('提示', '确定删除？', function(r) {
		if (r) {
			$.ajax({ url : _path + '/fullReduce/deleteFullDiscount?actId='+actId+'&ruleId='+ruleId ,type : "post" ,  datatype : "json" , success : function(data) {
