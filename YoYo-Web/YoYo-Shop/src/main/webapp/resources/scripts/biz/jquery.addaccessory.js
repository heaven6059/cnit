	...
	$.fn.addaccessory = function() {
		...
		return this
				.each(function() {
					...
					return Accessory.prototype[ method ] ? Accessory.prototype[ method ].apply(instance, arguments)
							: undefined;
				});
	};
	...
	Accessory.prototype = {
		...
		kreacteNormalPanel : function(data) {
			...
			if(data!=null && (data.accessoryGoods!=null && data.accessoryGoods!='')){
				...
				commonAjax( _path + '/product/findProductByIds',{'productId':accessoryGoods},function(data){
					...
					that.forms[ that.forms.length ] = { id : 'acc-group-' + that.forms.length + 1 };
				...
				$form.find('tr:last td').append(html);
				this.forms[ this.forms.length ] = { id : 'acc-group-' + this.forms.length + 1 };
		...
		kreacteGroupPanel : function(data) {
			...
			if(data){
				var $filter_1 = $('<div><table width="100%">'
						+ '<tr><th>按价格区间筛选</th><td><input type="text" name="pricefrom" value='+data.priceform+'> - <input type="text" name="priceto" value='+data.priceto+'> 元</td></tr>'
						+ '<tr><th>按商品关键词筛选</th> <td><input type="text" name="searchKeywords" value='+data.searchKeywords+'></td></tr>' + '</table></div>');
				var $filter_2 = $('<div><table style="width:auto;"><tr>'
						+ '<td><label for="f_cat">商品分类 </label><select style="width:300px" id="f_cat" name="cat_id[]"></select></td></tr><tr>'
						+ '<td><label for="f_brand">品牌 </label><select style="width:300px" id="f_brand" name="brand_id[]" multiple="multiple"></select></td>'
						/*+ '<td><label for="f_tag">标签 </label><select style="width:120px" id="f_tag" name="tag[]"></select></td>'*/
						+ '</tr></table></div>');
				...
				this.getBrandData(biz.rootPath() + '/brandTypeShip/findShip',$form.find('#f_brand'),brandIds);
			}else{
				var $filter_1 = $('<div><table width="100%">'
						+ '<tr><th>按价格区间筛选</th><td><input type="text" name="pricefrom" /> - <input type="text" name="priceto" /> 元</td></tr>'
						+ '<tr><th>按商品关键词筛选</th> <td><input type="text" name="searchKeywords"/></td></tr>' + '</table></div>');
				var $filter_2 = $('<div><table style="width:auto;"><tr>'
						+ '<td><label for="f_cat">商品分类 </label><select style="width:300px" id="f_cat" name="cat_id[]" ></select></td></tr><tr>'
						+ '<td><label for="f_brand">品牌 </label><select style="width:300px" id="f_brand" name="brand_id[]" multiple="multiple"></select></td>'
					/*	+ '<td><label for="f_tag">标签 </label><select style="width:120px" id="f_tag" name="tag[]"></select></td>'*/
						+ '</tr></table></div>');
				$form.find('tr:last td').append($filter_1).append($filter_2);
				this.getBrandData(biz.rootPath() + '/brandTypeShip/findShip',$form.find('#f_brand'),null);
			}
			$form.find('#f_cat').trapown(
					{ url : _path + '/comboBox/cateTreeCombox' , multiple : true , idField : 'catId' ,
						textField : 'catName' , parentField : 'parentCatId' , initParentId : 0,selected:catIds });
			
			/*$form.find('#f_tag').comdropdown(
		...
		commonKreate : function(data) {
			if(data){
				 var html='<form class="acc-group-form"><table width="100%">'
						...
						+ '<tr><th>最小购买量：</th><td><input type="text" name="minBuy" value='+data.minBuy+'></td></tr>'
						+ '<tr><th>最大购买量：</th><td><input type="text" name="maxBuy" value='+data.maxBuy+'></td></tr>';
						if(data.discType=='discount'){
							html+= '<tr><th>套装优惠：</th><td><input type="radio" name="discType" onclick="this.parentNode.parentNode.nextSibling.firstChild.innerHTML=\'<th>优惠折扣：</th>\';" value="discount" checked="checked">优惠某个折扣 <input type="radio" name="discType" value="minus" onclick="this.parentNode.parentNode.nextSibling.firstChild.innerHTML=\'<th>优惠金额：</th>\';">优惠一定金额</td></tr>';
						}else{
						...
						html+= '<tr><th>优惠额度：</th><td><input type="text" name="credit" value='+data.credit+'>(无优惠可不填；优惠9折就输入0.9，优惠100元就输入100)</td></tr>'
						+ '<tr><th>添加套装商品：</th><td></td></tr></table></form>';
				 return $(html);
			}else{
				return $('<form class="acc-group-form"><table width="100%">'
						+ '<tr><th>套装组名称：</th><td><input type="text" name="accGroupName"><a onclick="this.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.removeChild(this.parentNode.parentNode.parentNode.parentNode.parentNode)" class="easyui-linkbutton l-btn l-btn-small" group="" id=""><span class="l-btn-left"><span class="l-btn-text">删除此组套装</span></span></a></td></tr>'
						+ '<tr><th>最小购买量：</th><td><input type="text" name="minBuy"></td></tr>'
						+ '<tr><th>最大购买量：</th><td><input type="text" name="maxBuy"></td></tr>'
						+ '<tr><th>套装优惠：</th><td><input type="radio" name="discType" onclick="this.parentNode.parentNode.nextSibling.firstChild.innerHTML=\'<th>优惠折扣：</th>\';" value="discount" checked="checked">优惠某个折扣 <input type="radio" name="discType" value="minus" onclick="this.parentNode.parentNode.nextSibling.firstChild.innerHTML=\'<th>优惠金额：</th>\';">优惠一定金额</td></tr>'
						+ '<tr><th>优惠折扣：</th><td><input type="text" name="credit">(无优惠可不填；优惠9折就输入0.9，优惠100元就输入100)</td></tr>'
						+ '<tr><th>添加套装商品：</th><td></td></tr></table></form>');
			}
		...
		getAccessory:function(){
			var $this = this;
			commonAjax(_path+'/goods/getAccessoryGoods',{'goodsId':this.options.goodsId},function(data){
