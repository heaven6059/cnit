...
function loadAdverts() {
    ...
    if (ids.length > 0) {
        ...
        var retValue = request("projs", "http://pro.yangche51.com/AjaxPage/AdvertLoad20111026.aspx?w=" + status + "&encoding=" + encoding + "&advertSpaceIDs=" + ids);
    }
}

function loadAdvertbyID(id) {
   var name = [],
        encoding = (document.characterSet || document.charset) || "utf-8",
        status = screen.width >= 1280 ? "w" : "z";
   var retValue = request("projs", "http://pro.yangche51.com/AjaxPage/AdvertLoad20111026.aspx?w=" + status + "&encoding=" + encoding + "&advertSpaceIDs=" + id);
}
...
function request(id, url) {
    ...
    oScript = document.createElement("script"); oScript.setAttribute("src", url); oScript.setAttribute("id", id); oScript.setAttribute("charset", "uft-8"); oScript.setAttribute("type", "text/javascript"); oScript.setAttribute("language", "javascript"); head.appendChild(oScript);
    return oScript;
}
...
function Flash() {
    Flash.prototype.showFlash = function(contentID, flashFile, adhref, width, height) {
        var element = $(document.createElement("a")).attr({ id: "ad" + contentID, href: adhref });
        var params = { menu: "false", quality: "high", scale: "noscale", wmode: "transparent", allowFullscreen: "true", allowscriptaccess: "always", bgcolor: "#1D1D1D" };
