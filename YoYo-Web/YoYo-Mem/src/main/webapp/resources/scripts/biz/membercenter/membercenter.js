$(document).ready(function(){
	$("#all_orders>h4>a").click(function (){
		$(this).addClass("current").siblings().removeClass("current");
		switch ($(this).index()) {
		case 0:			
			loadDataList({status:"create"});
			break;
		case 1:
			loadDataList({status:"confirm"});
			break;
		case 2:
			loadDataList({status:"comment"});
			break;
		case 3:
			loadDataList({status:"refunds"});
			break;
		}
	});
	
	$('.clo_ds_le').on('click', function(){
		var left=($(".picscroll_buy>ul").position().left+912);
		if(left<=0){
			if(!$(".picscroll_buy>ul").is(":animated")) {
			$(".picscroll_buy>ul").animate({left:left},600);
			}
		}
	});
	
	$('.clo_ds_ri').on('click', function(){
		var right=($(".picscroll_buy>ul").position().left-912);
		if(($(".picscroll_buy").width()-$(".picscroll_buy>ul").width())<=right){
			if(!$(".picscroll_buy>ul").is(":animated")) {
			$(".picscroll_buy>ul").animate({left:right},600);
			}
		}
	});
	
	$(".gridlist").on("click",".finish-order",function (){
		...
		$.ajax({
			...
			success : function(returnData) {
				if (returnData.retCode == yoyo.successCode) {
					...
					layer.alert(msg,{title:false,closeBtn:false,icon:0});
				}				
			}
		});
	});
...
function loadDataList(param){
	var html=[];
	$(".gridlist tbody").remove();
	$.ajax({
		url:'../memberCenter/findOrderInfos',
		type:"post",
		data:param,
		dataType:"json",
		async: false,
			   ...
			   $.each(returnData.rows,function(index,order){
		             ...
		             var itemsLength=order.orderItems.length;
		               $.each(order.orderItems,function(index,orderItem){
		            		...
		            		html.push('单价：'+formatCurrency(orderItem.price,2)+'<br>');
		            		html.push('数量：'+orderItem.nums);
		            		html.push('</td>');
		            		if(index==0){
		            			html.push('<td rowspan="'+itemsLength+'" class="price">￥'+formatCurrency(order.finalAmount,2)+'</td>');		            			
