...
function loadDataList(opts){
	$.ajax({
		url:yoyo.memUrl+'/report/getReportList',
		data:{
			page:$("#page-num").val(),
			rows:20,
		...
		success:function(returnData){
			$("#tableId tr:not(:first)").remove();
			 ...
			 $.each(returnData.rows,function(index,reportDTO){
				 ...
				 html.push('<td align="center"><a class="btn-bj-hover operate-btn" href = "'+yoyo.memUrl+'/report/getReportDetailById?reportId='+reportDTO.reportId+'" >查看详情</a></td>');
				 html.push('</tr>');
			 });
