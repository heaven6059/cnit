...
<link rel="stylesheet" type="text/css" href="${path}/resources/styles/complain/complain.css?v=${versionVal}" />
...
<script src="${path}/resources/scripts/biz/order/complainDetail.js?v=${versionVal}"></script>
</head>
<body>
						...
						<c:if test="${fn:length(order.complainComments)>0}">
						  <c:forEach items="${order.complainComments}" var="complainComment">
							<div class="row-item first-row-item odd">								
								<c:if test="${complainComment.source=='buyer'}"><div class="ctitle avator">自己：</div></c:if>
								<c:if test="${complainComment.source=='seller'}"><div class="ctitle avator">卖家：</div></c:if>
								<div class="info">
									<div class="action">
			...
			<div class="col-sub">
				<div class="request-info boxbase J_RequestInfoWrap ">
					...
					<div class="box-hd">
						...
						<c:if test="${order.status eq 'intervene'}">
						<div class="box-act">
							<a href="javascript:cancelComplain(${order.complainId});" class="">撤销</a>
						</div>
						</c:if>
					</div>
					<div class="box-bd J_RequestInfo">
						...
						<div class="row-item ">
							...
							<div class="info">${order.storeName }</div>
						</div>
						<div class="row-item ">
							<div class="ctitle">
								<label>申请时间：</label>
				...
				<div id="J_BabyDetail" class="boxbase baby-detail J_BabyDetail">
					...
					<div class="box-bd">
						<div class="row-item first-row-item">
							<div class="ctitle">
								<a target="_blank" href="${portalPath}/goodsManager/goodsIndex?goodsId=${orderItem.product.goodsId}"> 
									<img width="252px" height="252px" src="${orderItem.product.picturePath}" class="goods-img" alt="${orderItem.product.name}">
								</a> 
								<label class=""></label>
							</div>
							<div class="info">
						...
						<div class="row-item">
							...
							<div class="info">
								<a target="_blank" href="${portalPath}/shop/index?companyId=${order.companyId}">${order.companyName}</a>
								<span>
								</span>
							</div>
						</div>
						...
						<div class="row-item last-row-item">
							<a  href="${path}/memberOrder/viewOrder?orderId=${orderItem.orderId}" target="_blank" id="J_MoreBabyTrigger" class="more">查看订单信息<s class="spt-icon"></s></a>
						</div>
					</div>
					</c:forEach >
				</div>
