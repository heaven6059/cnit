...
<body>
	<div class="shop_manager_right ">
		...
		<form  id="reshipForm" class="section">
			...
			<div id="gEditor-Body">
				<div class="title_fb ">填写售后申请基本信息</div>
				<div class="FormWrap" style="background: none">
					...
					<div class="store_title">
						<label><span style="color: red; ">*温馨提示：</span> 本次售后服务将由 卖家<span style="color: red; "> ${order.store.storeName}</span> 为您提供</label>
					</div>
					<table width="100%" border="0" cellspacing="0" cellpadding="0" class="liststyle data width1" style="border: none">
						...
						<c:if test="${order.isCoupons eq 1}">
						<tr>
							<th>优惠金额：</th>
							<td>
								<label>${order.discountValue}</label>
							</td>
						</tr>
						</c:if>
						...
						<tr class="hide" id="amount_tr">
							...
							<td>
								<span id="amount_span">${(order.orderItems[0].price-order.discountValue*(order.orderItems[0].price/order.totalAmount))}</span>
								<c:if test="${order.isCoupons eq 1}"><p>注意：最大可退金额=需退款商品原价-订单中优惠的金额*（需退款商品原价/订单原价）。</p></c:if>
							...
							<td>
								<div class="fl">
									<div class="quantity-form">
										...
										<c:if test="${order.isCoupons ne 1}">
										<input type="text" class="text01" value="1" data-price="${order.orderItems[0].price}" data-nums="${order.orderItems[0].nums}" name="nums" id="number" readonly="readonly" maxlength="2"/>
										</c:if>
										<c:if test="${order.isCoupons eq 1}">
										<input type="text" class="text01" value="1" data-price="${(order.orderItems[0].price-order.discountValue*(order.orderItems[0].price/order.totalAmount))}" data-nums="${order.orderItems[0].nums}" name="nums" id="number" readonly="readonly" maxlength="2"/>
										</c:if>
