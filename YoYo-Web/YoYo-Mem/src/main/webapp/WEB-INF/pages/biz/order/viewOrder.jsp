...
<link type="text/css" href="${path}/resources/styles/order/orderdetail.css?v=${versionVal}" rel="stylesheet" />
...
<body>
							...
							<ul>
								...
								<li>订单状态：
									<span class="siteparttitle-blue">
										<c:if test="${order.status eq 'create'}">未付款</c:if>
			...
			<div style="border: none" class="order-track">
				...
				<div id="orderinfo">
					...
					<div class="mc">
						...
						<c:if test="${!empty order.store.storeName}">
							<dl>
								<dt>店铺信息</dt>
								<dd>
									<ul>
										...
										<li>店铺地址：${order.store.area} ${order.store.addr}</li>
									</ul>
								</dd>
							</dl>
						</c:if>
											...
											<td>
												<div class="img-list">
													<a href="${portalPath}/goodsManager/goodsIndex?goodsId=${orderItem.product.goodsId}" target="_blank" class="img-box"> 
														<img width="80" height="80" src="${orderItem.product.picturePath}" />
													</a>
												</div>
											</td>
											<td>
	...
	<script type="text/javascript">
		$(function (){
			$(".img-box img").each(function (x,item){
